...
if (!function_exists('becko_stock_request_code')) {
    function becko_stock_request_code(int $length = 10, bool $capitalize = false){
        $db = \Config\Database::connect();
        $code = "";
        for ($i=0; $i <20 ; $i++) { 
...
if (!function_exists('becko_purchase_request_code')) {
    function becko_purchase_request_code(int $length = 10, bool $capitalize = false){
        $db = \Config\Database::connect();
        $code = "";
        for ($i=0; $i <20 ; $i++) { 
            $gets = generateRandomString($length,$capitalize);
...
if (!function_exists('becko_purchase_order_code')) {
    function becko_purchase_order_code(int $length = 10, bool $capitalize = false){
        $db = \Config\Database::connect();
        $code = "";
        for ($i=0; $i <20 ; $i++) { 
            $gets = generateRandomString($length,$capitalize);
...
if (!function_exists('becko_supplier_code')) {
    function becko_supplier_code(int $length = 10, bool $capitalize = false){
        $db = \Config\Database::connect();
        $code = "";
        for ($i=0; $i <20 ; $i++) { 
            $gets = generateRandomString($length,$capitalize);
...
if (!function_exists('becko_taktikal_code')) {
    function becko_taktikal_code(int $length = 10, bool $capitalize = false){
        $db = \Config\Database::connect();
        $code = "";
        for ($i=0; $i <20 ; $i++) { 
            $gets = generateRandomString($length,$capitalize);
...
if(!function_exists('getSequenceTransac')){
    function getSequenceTransac($location){
        $year = date('Y');
        $db = \Config\Database::connect();
        $builder = $db->table('pos_transactions');
        if($location !== ''){
            $builder->where('pos_id', $location);
        }
        $builder->where('YEAR(transaction_date)', $year);
        $builder->selectCount('id', 'total_ids'); // Count 'id' column and alias it as 'total_ids'
        $query = $builder->get();
        $number = $query->getRow()->total_ids;
        return $number;
    }
}
