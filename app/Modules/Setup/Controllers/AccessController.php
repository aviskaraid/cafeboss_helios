            ...
            if($post['parent_id']!="0" && \strlen($post['modules'])==0){
               return redirect()->back()->withInput()->with('error', "Module not Found");
                ...
                if($parent_id=="0"){
                    ...
                    $formChange['module_name'] = strtolower(str_replace(" ","",$trimString));
                }
                if($parent_id!="0"){
                    $trimString = trim($post['label_name']);
                    $formChange['function_name'] = strtolower(str_replace(" ","",$trimString));
                }
                $formChange['label_name'] = trim($post['label_name']);
                $formChange['url'] = strtolower(trim($post['url']));
                $formChange['parent_id'] = (int)$parent_id;
                $formChange['main_module_id'] = (int)$main_module_id;
                $formChange['active'] = 1;
                $formChange['icon_image'] = strtolower($post['icon_image']);
                $addSave = $access->update($id,$formChange);
                if(!$addSave) {
			       return redirect()->back()->withInput()->with('errors', $access->errors());
                } else {
                    return redirect()->to(site_url('settings/accessmenu'))->with('success', 'Data Berhasil Disimpan');
