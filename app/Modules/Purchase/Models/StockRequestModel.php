...
{
    protected $table      = 'stock_request'; // Table name
    ...
    public function getPaginated($num, $keyword = null, $status = 1) {
        ...
        $builder->join("users b", "b.id = stock_request.requester_id", "left");
        $builder->select("stock_request.*,b.fullname as requester_name");
        if($keyword != '') {
            $builder->orLike('ref_no', $keyword);
            $builder->orLike('transaction_date', $keyword);
        }
        return [
            'stockrequest' => $this->paginate($num),
            'pager' => $this->pager,
        ];
    }
