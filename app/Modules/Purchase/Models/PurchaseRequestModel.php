...
{
    protected $table      = 'purchase_request'; // Table name
    ...
    public function getPaginated($num, $keyword = null, $status = 1) {
        ...
        $builder->join("users a","a.id = purchase_request.requester_id","LEFT");
        $builder->join("users b","b.id = purchase_request.approval_id","LEFT");
        $builder->select("purchase_request.*,a.fullname as requester_name,b.fullname as approval_name");
        if($keyword != '') {
            $builder->orLike('ref_no', $keyword);
            $builder->orLike('request_id', $keyword);
        }
        return [
            'purchaserequest' => $this->paginate($num),
            'pager' => $this->pager,
        ];
    }
