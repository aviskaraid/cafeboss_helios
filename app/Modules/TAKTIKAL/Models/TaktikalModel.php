...
class TaktikalModel extends Model
{
    protected $table      = 'taktikal'; // Table name
    ...
    protected $allowedFields = ["ref_no",'ref_code',"supplier_id","description",
                                "detail","status","start_time","end_time","active","created_by","updated_by",
                                "deleted_by"];
    ...
    protected function callBackDelete (array $data){
        ...
        $builder = $this->builder();
        $builder->select('taktikal.*,a.name as supplier_name');
        $builder->join('supplier a', 'a.id = taktikal.supplier_id');
        if($keyword != ''){
            $builder->groupStart();
            $builder->like('taktikal.ref_no', $keyword);
            $builder->orLike('taktikal.detail', $keyword);
            $builder->orLike('taktikal.description', $keyword);
            $builder->groupEnd();
        }
       
        $builder->groupBy( ['taktikal.id']);
        return [
