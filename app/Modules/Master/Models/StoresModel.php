...
{
    protected $table      = 'stores'; // Table name
    ...
    protected $allowedFields = ["business_id","code","name","description","address","phone_number",
                                "location","remark","type","picture","active",
    ...
    public function insertStoreCategoryMap($id,$data){
        $db = \Config\Database::connect();
        foreach ($data as $item) {
            $builder = $db->table('store_category_map'); // Specify the other table
            $builder->where("store_id",$item['store_id']);
            $builder->where("category_id",$item['category_id']);
            $record = $builder->get()->getRow();
            if ($record) {
                $builder->where('store_id', $id)
                ->delete();
            }
            $db->transCommit();
            foreach ($data as $item) {
    ...
    public function insertSetup($data){
        $db = \Config\Database::connect();
        $builder = $db->table('store_setup');     
        $db->transCommit();
        $builder->insert($data);
        $db->transComplete();
    }

    public function edit_setup_change($id,$data){
        $db = \Config\Database::connect();
        $builder = $db->table('store_setup');     
        $db->transCommit();
        $builder->update($data,["store_id"=>$id]);
        $db->transComplete();
    }
