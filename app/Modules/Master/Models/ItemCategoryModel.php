...
class ItemCategoryModel extends Model
{
    ...
    protected $allowedFields = ["name","label_name","parent_id","slug","category_type","active","picture"];
    ...
    public function getPaginated($num, $keyword = null, $status = 1) {
        ...
        $builder->join("item_category parent", "parent.id = item_category.parent_id", "left");
        $builder->select("item_category.*,
        parent.label_name AS parent_name");
        $builder->orderBy("COALESCE(parent.id, zax_item_category.id),zax_item_category.id");
        if($keyword != '') {
