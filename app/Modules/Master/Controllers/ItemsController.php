...
{   
    ...
    public function edit($id = null){
        $getData = new ItemsModel();
        $where = ['id'=>$id];
        $get = $getData->getFind($where,null,null,0,0,[],[])[0];
        $data['title'] = "Edit ( ".$get->display_name." )";
        $data['data'] = $get;
        //dd($get);
        return hmvcView($this->module, 'items_edit', $data);
    }
    ...
    public function create_process(){
        ...
        $supplier = explode(",",$post['input_supplier']);
        $add_item =[
            "sku"           =>$post['sku'],
            "code"          =>$post['code'],
            "name"          =>$post['name'],
        ...
        foreach ($supplier as $item) {
            ...
            array_push($supplierID,$set);
        }
        $items->insertItemSupplierMap($itemID,$supplierID);
    ...
    public function changes($id = null){
        ...
        $items->update($post['item_id'],$add_item);
        $location =[];
        foreach ($warehouse as $item) {
            $set = [
        ...
        $items->insertItemSupplierMap($post['item_id'],$supplierID);
        $items->insertLocation($post['item_id'],$location);
        $items->insertPrice($post['item_id'],$price);
        $items->insertStock($post['item_id'],$stock);
        $data['item'] = $add_item;
        $data['redirect'] = "master/items";
        return json_response($data,200,$message);
    }
