...
let catId =  $("#cat_id").length > 0 ? document.getElementById("cat_id").value : 0;
$('#category').select2({
...
$('#category').on('select2:select', function (e) {
  ...
  $('#itemCategory_Create').on('submit', function(e) {
          ...
          Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(base_url +'master/foodmenucategory/create_process', { // Replace '/auth/login' with your CI4 login route
                        method: 'POST',
                        headers: {
                                ...
                                Swal.fire({
                                  ...
                                  console.log(data);
                                    Swal.fire({
                                        ...
                                        timer: 2500,
                                        willClose: () => {
                                            window.location.replace("/master/foodmenucategory");
                                        }
                                    });
                                }
                        })
  ...
  $('#itemCategory_Changes').on('submit', function(e) {
            const myForm = document.querySelector('#itemCategory_Changes');
          e.preventDefault();
          Swal.fire({
                  title: 'Are you sure you Want Proceed Form ? ',
                  text: "You won't be able to revert this!",
                ...
                if (result.isConfirmed) {
                    ...
                    const formData = new FormData(myForm);
                    const data = Object.fromEntries(formData.entries());
                    fetch(base_url +'master/foodmenucategory/changes/'+$('#cat_id').val(), { // Replace '/auth/login' with your CI4 login route
                        method: 'PATCH',
                        headers: {
                            'Content-Type': 'application/json'
                        },
