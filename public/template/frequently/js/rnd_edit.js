...
`       );
    ...
    $(document).ready(function() {
        ...
        $('#rnd_edit').on('submit', function(e) {
                const myForm = document.querySelector('#rnd_edit');
                e.preventDefault();
                ...
                Swal.fire({
                        ...
                        fetch(base_url +'rnd/changes/'+items_id, { // Replace '/auth/login' with your CI4 login route
                            method: 'PATCH',
                            headers: {
    ...
    $(document).ready(function() {
        fetch(base_url+'apis/get_itemingredient_byItemId?id='+items_id)
        .then(response => response.json())
        .then(data => {
            var employeeOption = new Option(data.employee_name, data.employee_id, true, true);
            $('#item_employee').append(employeeOption).trigger('change');
            var ingredients = data.ingredients;
            if(ingredients.length>0){
                console.log("Exist Units "+ingredients.length );
                for (let index = 0; index < ingredients.length; index++) {
                    const item = ingredients[index];
                    const table = document.getElementById('myTable'); // Replace 'myTable' with your table's ID
                   // const tbody = table.getElementsByTagName('tbody')[index];
                    const newRowId = `${index+1}`;
                    $("#detail_list").append(`
                        <tr id="detail-${newRowId}">
                            <td style="width: 2%;display:none" style="display:none;">
                                <input type="hidden" name="rnd_id_${newRowId}" value="${newRowId}">
                                <input type="hidden" name="item_id_${newRowId}" id="item_id-${newRowId}">
                            </td>
                            <td style="width: 20%"> 
                                <select class="ingredient-${newRowId} form-control " id="ingredient_-${newRowId}"></select>
                            </td>
                            ...
                            <td style="width: 20%">
                                <input type="number" name="consumption_${newRowId}" class="form-control value_consumption" step="0.01" id="consumption_ingredient-${newRowId}">
                            </td>
                            <td style="width: 20%"> 
                                <input type="text" name="price_${newRowId}" class="form-control value_price" id="price_ingredient-${newRowId}">
                            </td>
                            <td style="width: 20%">
                                <input type="text" name="total_${newRowId}" class="form-control value_total" id="total_ingredient-${newRowId}" readonly>
                            </td>
                            <td style="width: 10%"><button type="button" class="btn btn-danger btn-sm remove-detail" data-id="${newRowId}">Remove</button></td>
                        </tr>
            `       );
                    ...
                    var qty = document.getElementById("consumption_ingredient-"+newRowId);
                    var total = document.getElementById("total_ingredient-"+newRowId);
                    qty.value = parseFloat(item.consumption);
                    total.value = numberFormat(parseFloat(item.total));
                    price.value = numberFormat(parseFloat(item.cost));
                    price.addEventListener('keyup', function(event) {
                        if (event.which >= 37 && event.which <= 40) {
                        event.preventDefault();
                            return;
                  ...
                  //  input.value = item.item_id
                    initializeSelect2(newSelect,input,unit,price,null,null);
                    var newOption = new Option(item.description, item.id, true, true);
                    newSelect.append(newOption).trigger('change');
                    unit.value = item.main_unit
                    input.value = item.item_id
                }
            }else{
